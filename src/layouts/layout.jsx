    ...
    <>
      ...
      <div className="flex h-[100dvh] overflow-hidden bg-[#f1f6f5]">
        <div className="hidden lg:block">
          <SidebarNav currentPath={location.pathname} handleLogout={handleLogout} />
        </div>

        <div className="lg:hidden">
          <SidebarNav currentPath={location.pathname} handleLogout={handleLogout} isMobileOpen={sidebarOpen} onCloseMobile={() => setSidebarOpen(false)} />
        </div>
        ...
        <main className="relative flex-1 overflow-auto p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-6 min-w-0">
          <div className="absolute left-3 top-3 z-20 lg:hidden">
            <button onClick={() => setSidebarOpen(true)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-teal-200 bg-white text-[#1C7773] shadow-sm active:bg-teal-50" aria-label="Open menu">
        ...
        {sidebarOpen && (
          <button className="fixed inset-0 bg-black/40 z-30 lg:hidden" onClick={() => setSidebarOpen(false)} aria-label="Close menu" />
        )}
        <ConfirmModal
          isOpen={logoutConfirmOpen}
          onClose={() => { if (!logoutLoading) setLogoutConfirmOpen(false); }}
