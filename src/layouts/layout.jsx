...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    syncPrototypeData().catch(() => {}).finally(() => { if (active) setSharedDataReady(true); });
    return () => { active = false; };
        ...
        <main className="relative flex min-w-0 flex-1 flex-col overflow-hidden p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-6">
          ...
          <AnimatePresence mode="wait" initial>
            <motion.div key={location.pathname} initial={pageMotion.initial} animate={pageMotion.animate} exit={pageMotion.exit} transition={pageMotion.transition} className="no-default-transition min-h-0 flex-1 overflow-hidden">
              {sharedDataReady ? (children || <Outlet />) : <motion.div initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="flex h-full items-center justify-center text-sm text-gray-400">Syncing shared business data...</motion.div>}
            </motion.div>
          </AnimatePresence>
        </main>
