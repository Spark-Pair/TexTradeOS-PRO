...
export const fetchPurchaseArticleUsage = async (articleNo, purchaseNumber) => unwrap(await apiClient.get(`/purchases/articles/${encodeURIComponent(articleNo)}/usage`, { params: { purchase_number: purchaseNumber } }));

export const fetchInventory = async () => unwrap(await apiClient.get("/inventory"));
export const fetchInventoryMovements = async (articleNo, purchaseNumber) => unwrap(await apiClient.get(`/inventory/${encodeURIComponent(articleNo)}/movements`, { params: { purchase_number: purchaseNumber } }));
