...
export default function Purchases() {
  ...
  const refresh = () => { setPurchases(listPurchases()); setSuppliers(listSuppliers()); };
  ...
  const filteredPurchases = useMemo(() => {
    ...
    return purchases.filter((purchase) => {
      ...
      return (!supplierName || purchase.supplier_name.toLowerCase().includes(supplierName)) && (!from || purchaseTime >= from) && (!to || purchaseTime <= to);
    });
  }, [filters, purchases]);
  ...
  const stats = useMemo(() => {
    ...
    return { totalPurchases: purchases.length, packetCount, totalAmount, avgPurchase: purchases.length ? totalAmount / purchases.length : 0 };
  }, [purchases]);

  const handleSubmit = async (payload) => {
    try {
      const saved = await savePurchase(payload);
      refresh();
      showToast({ type: "success", message: payload._id ? "Purchase updated successfully" : "Purchase saved successfully" });
      return saved;
    } catch (error) {
      showToast({ type: "error", message: errorMessage(error, "Could not save purchase") });
      throw error;
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast({ type: "error", message: errorMessage(error, "Could not delete purchase") });
    ...
    <>
      <div className="relative z-10 max-w-7xl mx-auto h-full flex flex-col">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          <StatCard label="Total Purchases" value={formatNumbers(stats.totalPurchases, 0)} icon={ReceiptText} /><StatCard label="Packet Labels" value={formatNumbers(stats.packetCount, 0)} icon={Package} variant="warning" /><StatCard label="Total Amount" value={formatNumbers(stats.totalAmount, 2)} icon={Wallet} variant="success" /><StatCard label="Avg Purchase" value={formatNumbers(stats.avgPurchase, 2)} icon={Sigma} />
        </div>
      ...
      <PurchaseDetailsModal purchase={detailsPurchase} onClose={() => setDetailsPurchase(null)} onPrintLabels={() => { setFormModal({ isOpen: true, purchase: detailsPurchase, initialStep: "labels" }); setDetailsPurchase(null); }} />
      ...
      <ConfirmModal isOpen={Boolean(deleteTarget)} onClose={() => { if (!isDeleting) setDeleteTarget(null); }} onConfirm={handleDelete} isLoading={isDeleting} closeOnConfirm={false} title="Delete Purchase" message={`Delete purchase "${deleteTarget?.purchase_number || ""}"? This cannot be undone.`} confirmText="Delete Purchase" variant="danger" />
    </>
  );
}
