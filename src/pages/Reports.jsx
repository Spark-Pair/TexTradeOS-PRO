...
export default function Reports() {
  ...
  return <div className="reports-page relative z-10 mx-auto flex h-full max-w-7xl flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-auto pb-8">
      ...
      {data && <section className="mt-5 overflow-hidden rounded-3xl border border-gray-300 bg-white"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 px-5 py-4 sm:px-6"><div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-lg bg-teal-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#127475]">{category}</span><p className="truncate text-sm font-bold text-gray-900">{selectedName}</p></div><p className="mt-1 flex items-center gap-1.5 text-xs text-gray-500"><CalendarRange size={13}/>{data.period?.full_statement ? "Full account statement" : `${data.period?.date_from || "Beginning"} to ${data.period?.date_to || "Present"}`} · {data.rows?.length || 0} transactions</p></div><div className="flex flex-wrap gap-2"><Button outline icon={Printer} onClick={print}>Print</Button><Button icon={FileDown} onClick={exportPdf}>Save PDF</Button></div></div><div className="grid grid-cols-2 gap-px border-b border-gray-200 bg-gray-200 lg:grid-cols-4"><Summary label="Opening Balance" value={data.opening_balance}/><Summary label="Total Debit" value={data.totals?.debit}/><Summary label="Total Credit" value={data.totals?.credit}/><Summary label="Closing Balance" value={data.closing_balance} highlight/></div><div className="statement-preview-stage overflow-auto bg-[#e9eeed] p-4 sm:p-6 lg:p-8"><div className="statement-sheet mx-auto min-h-[297mm] w-[210mm] max-w-none bg-white px-[12mm] py-[11mm] shadow-[0_12px_40px_rgba(15,23,42,0.14)]"><StatementPaper data={data} type={category}/></div></div></section>}
    </div>
    <style>{`@media print{@page{size:A4 portrait;margin:11mm 10mm 12mm}html,body,#root{width:100%!important;height:auto!important;min-height:0!important;margin:0!important;padding:0!important;overflow:visible!important;background:#fff!important}body *{visibility:hidden!important}.statement-sheet,.statement-sheet *{visibility:visible!important}.statement-sheet{position:absolute!important;left:0!important;top:0!important;box-sizing:border-box!important;width:100%!important;min-height:0!important;margin:0!important;padding:0!important;background:#fff!important;box-shadow:none!important;overflow:visible!important}.statement-paper{width:100%!important;margin:0!important;padding:0!important;background:#fff!important;color:#111827!important;overflow:visible!important}.statement-preview-stage{overflow:visible!important;background:#fff!important;padding:0!important}.statement-table-wrap{overflow:visible!important}.statement-paper table{width:100%!important;table-layout:fixed!important}.statement-paper thead{display:table-header-group!important}.statement-paper tfoot{display:table-footer-group!important}.statement-row{break-inside:avoid!important;page-break-inside:avoid!important}.statement-header,.statement-party{break-after:avoid!important;page-break-after:avoid!important}.statement-totals{break-inside:avoid!important;page-break-inside:avoid!important}.statement-footer{break-inside:avoid!important;page-break-inside:avoid!important}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}`}</style>
  </div>;
}
