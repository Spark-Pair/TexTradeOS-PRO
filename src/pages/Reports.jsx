  ...
  return <div className="reports-page relative z-10 mx-auto flex h-full max-w-7xl flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-auto pb-8">
      <section className="overflow-visible rounded-3xl border border-gray-300 bg-white">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 px-5 py-4 sm:px-6">
          <div><div className="flex items-center gap-2"><div className="rounded-xl bg-teal-50 p-2 text-[#127475]"><FileText size={18}/></div><div><h2 className="text-sm font-bold text-gray-900">Account Statement</h2><p className="mt-0.5 text-xs text-gray-500">Select an account and optional reporting period.</p></div></div></div>
          <div className="rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-[11px] font-medium text-gray-500">Dates optional · blank dates generate full history</div>
        </div>
        <div className="grid gap-4 p-5 sm:p-6 xl:grid-cols-[180px_minmax(260px,1fr)_170px_170px_auto]">
          <Select label="Category" options={CATEGORY_OPTIONS} value={category} onChange={(value) => { setCategory(value); clearPreview(); }} />
          <Select label={category === "customer" ? "Customer" : "Supplier"} options={partyOptions} value={partyId} disabled={partiesLoading} placeholder={partiesLoading ? "Loading accounts..." : `Select ${category}`} onChange={(value) => { setPartyId(value); clearPreview(); setError(""); }} />
          <DateField label="From Date" value={dateFrom} onChange={(value) => { setDateFrom(value); clearPreview(); }} />
          <DateField label="To Date" value={dateTo} onChange={(value) => { setDateTo(value); clearPreview(); }} />
          <div className="flex items-end"><Button icon={Search} onClick={generate} disabled={loading || partiesLoading} className="w-full xl:min-w-32">{loading ? "Generating..." : "Generate"}</Button></div>
        </div>
      ...
      {data && <section className="mt-5 overflow-hidden rounded-3xl border border-gray-300 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 px-5 py-4 sm:px-6">
          <div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-lg bg-teal-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#127475]">{category}</span><p className="truncate text-sm font-bold text-gray-900">{selectedName}</p></div><p className="mt-1 flex items-center gap-1.5 text-xs text-gray-500"><CalendarRange size={13}/>{data.period?.full_statement ? "Full account statement" : `${data.period?.date_from || "Beginning"} to ${data.period?.date_to || "Present"}`} · {data.rows?.length || 0} transactions</p></div>
          <div className="flex flex-wrap gap-2"><Button outline icon={Printer} onClick={print}>Print</Button><Button icon={FileDown} onClick={exportPdf}>Export PDF</Button></div>
        </div>
        <div className="grid grid-cols-2 gap-px border-b border-gray-200 bg-gray-200 lg:grid-cols-4"><Summary label="Opening Balance" value={data.opening_balance}/><Summary label="Total Debit" value={data.totals?.debit}/><Summary label="Total Credit" value={data.totals?.credit}/><Summary label="Closing Balance" value={data.closing_balance} highlight/></div>
        <div className="statement-preview-stage overflow-auto bg-[#e9eeed] p-4 sm:p-6 lg:p-8"><div className="statement-sheet mx-auto w-[297mm] min-h-[210mm] max-w-none bg-white p-[11mm] shadow-[0_12px_40px_rgba(15,23,42,0.14)]"><StatementPaper data={data} type={category}/></div></div>
      </section>}
    </div>
    <style>{`@media print{html,body,#root{width:100%!important;height:auto!important;min-height:0!important;margin:0!important;padding:0!important;overflow:visible!important;background:#fff!important}.reports-page{display:block!important;width:100%!important;max-width:none!important;height:auto!important;margin:0!important;padding:0!important}.reports-page>*{display:none!important}.reports-page>div{display:block!important;overflow:visible!important;padding:0!important}.reports-page>div>*{display:none!important}.reports-page>div>section:last-of-type{display:block!important;margin:0!important;border:0!important;border-radius:0!important;overflow:visible!important;background:#fff!important}.reports-page>div>section:last-of-type>*{display:none!important}.reports-page .statement-preview-stage{display:block!important;overflow:visible!important;background:#fff!important;padding:0!important}.reports-page .statement-sheet{display:block!important;width:auto!important;min-height:0!important;margin:0!important;padding:0!important;box-shadow:none!important}.statement-paper{display:block!important;width:100%!important;margin:0!important;padding:0!important;background:#fff!important;color:#0f172a!important}.statement-paper table{width:100%!important}.statement-paper thead{display:table-header-group!important}.statement-paper tfoot{display:table-footer-group!important}.statement-row{break-inside:avoid!important;page-break-inside:avoid!important}.statement-header{break-after:avoid!important;page-break-after:avoid!important}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@page{size:A4 landscape;margin:10mm}}`}</style>
  </div>;
}
