import { ArchiveRestore, Database, Download, HardDrive, LoaderCircle, RefreshCcw, Upload } from "lucide-react";
...
export default function BackupPage() {
  ...
  const runCommand = async (type, payload = {}) => {
    ...
    try {
      ...
      showToast({ type: "error", message: error?.response?.data?.message || error.message || "Backup operation failed" });
    } finally { setBusy(""); }
  };
  ...
  const restoreFromComputer = async (event) => {
    ...
    event.target.value = "";
    if (!file || busy || !window.confirm(`Replace the current database with ${file.name}?`)) return;
  ...
  return (
    <div className="relative z-10 mx-auto flex h-full min-h-0 max-w-7xl flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-8">
        <div className="grid shrink-0 gap-4 md:grid-cols-2">
          <section className="rounded-3xl border border-teal-200 bg-teal-50/60 p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <Button icon={backupBusy ? LoaderCircle : Database} loading={backupBusy} disabled={Boolean(busy) && !backupBusy} onClick={() => runCommand("backup")}>
                {backupBusy ? "Creating Backup..." : "Create Backup"}
              </Button>
            </div>
            <label className={`mt-4 inline-flex items-center gap-2 text-sm font-medium ${busy ? "cursor-not-allowed text-gray-400" : "cursor-pointer text-teal-700 hover:underline"}`}>
              <Upload size={15} />
        ...
        <section className="mt-4 overflow-hidden rounded-3xl border border-gray-300 bg-white">
              ...
              {backups.map((backup) => (
                <div key={backup.name} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 transition-colors hover:bg-gray-50/80 sm:px-6">
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="sm" outline icon={Download} loading={busy === `download:${backup.name}`} disabled={Boolean(busy) && busy !== `download:${backup.name}`} onClick={() => downloadBackup(backup)}>Download</Button>
                    <Button size="sm" outline variant="warning" icon={ArchiveRestore} loading={busy === "restore"} disabled={Boolean(busy) && busy !== "restore"} onClick={() => { if (window.confirm("Replace the current database with this backup?")) runCommand("restore", { backup: backup.name }); }}>Restore</Button>
                  </div>
                </div>
              ))}
            </div>
