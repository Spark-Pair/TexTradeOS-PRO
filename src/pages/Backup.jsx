import { ArchiveRestore, Database, HardDrive, RefreshCcw } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import PageHeader from "../components/PageHeader";
import Button from "../components/Button";
import { useToast } from "../context/ToastContext";
import { fetchSystemCommand, fetchSystemStatus, requestSystemCommand, waitForCommand } from "../api/system";
...
export default function BackupPage() {
  ...
  return (
    <div className="relative z-10 mx-auto flex h-full min-h-0 max-w-7xl flex-col">
      <PageHeader
        title="Backup"
        subtitle="Create and restore local database snapshots."
        rightContent={<Button outline icon={RefreshCcw} onClick={refresh}>Refresh</Button>}
      />
      ...
      <section className="mt-4 flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-gray-300 bg-white">
        <div className="flex shrink-0 items-center gap-3 border-b border-gray-200 bg-gray-50 px-5 py-4 sm:px-6">
          ...
          <div>
            <h2 className="text-sm font-semibold text-gray-800">Available backups</h2>
            <p className="mt-0.5 text-xs text-gray-400">Restore a previous database snapshot when needed.</p>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          {loading ? (
            <div className="py-12 text-center text-sm text-gray-400">Loading backups...</div>
          ) : backups.length === 0 ? (
            <div className="py-12 text-center text-sm text-gray-400">No backups created yet.</div>
          ) : (
              ...
              {backups.map((backup) => (
                <div key={backup.name} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 transition-colors hover:bg-gray-50/80 sm:px-6">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-gray-800">{backup.name}</p>
                    <p className="mt-0.5 text-xs text-gray-400">{formatBytes(backup.size)}</p>
                  </div>
                  <Button
                    size="sm"
                    outline
                    ...
                    onClick={() => {
                      if (window.confirm("Replace the current database with this backup?")) runCommand("restore", { backup: backup.name });
                    }}
                  >Restore</Button>
                </div>
              ))}
