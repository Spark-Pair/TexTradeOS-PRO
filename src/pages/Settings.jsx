...
export default function SettingsPage() {
  ...
  return (
    <div className="relative z-10 mx-auto max-w-7xl pb-8">
      <PageHeader title="Settings" subtitle="Manage access rules and system preferences." rightContent={<Button icon={ShieldCheck} onClick={handleSave} disabled={loading || saving}>{saving ? "Saving..." : "Save Access Rules"}</Button>} />
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="overflow-hidden rounded-3xl border border-gray-300 bg-white">
          <div className="flex items-center gap-3 border-b border-gray-200 bg-gray-50 px-5 py-4 sm:px-6">
            <span className="grid h-9 w-9 place-items-center rounded-xl border border-teal-200 bg-teal-50 text-teal-700"><ShieldCheck size={17} /></span>
                  ...
                  <div key={`access-rule-${rule.key}`} className="px-5 py-4 transition-colors hover:bg-gray-50/70 sm:px-6">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                      <div className="flex min-w-0 flex-1 items-center gap-3">
                        <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${meta.tint}`}><Icon size={18} /></span>
        ...
        <aside className="space-y-4 xl:sticky xl:top-0">
          <div className="rounded-3xl border border-violet-200 bg-violet-50/60 p-5">
            <span className="grid h-10 w-10 place-items-center rounded-xl border border-violet-200 bg-white text-violet-700"><SlidersHorizontal size={18} /></span>
          ...
          <div className="rounded-3xl border border-amber-200 bg-amber-50/60 p-5">
            <p className="text-xs font-medium uppercase tracking-wide text-amber-700">Tip</p>
      ...
      {(user?.role === "developer" || user?.role === "admin") && <div className="mt-5"><SystemManagement /></div>}
    </div>
  );
}
