...
import { CircleCheck, Edit3, MoreVertical, Plus, Store, UserRoundCheck, XCircle } from "lucide-react";
...
export default function Suppliers() {
  ...
  const filteredSuppliers = useMemo(() => {
    ...
    return suppliers.filter((supplier) => {
      const matchesName = !name
        || supplier.supplier_name.toLowerCase().includes(name)
        || supplier.person_name.toLowerCase().includes(name);
  ...
  const filterConfig = [
    {
      label: "Supplier",
      placeholder: "Search supplier or person",
      type: "text",
      value: filters.name,
  ...
  return (
    <>
      <div className="relative z-10 max-w-7xl mx-auto h-full flex flex-col">
        <PageHeader title="Suppliers" subtitle="Create and manage supplier profiles." actionLabel="Add Supplier" actionIcon={Plus} onAction={() => setFormModal({ isOpen: true, supplier: null })} />

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <StatCard label="Total Suppliers" value={stats.total} icon={Store} />
        ...
        <div className="rounded-3xl bg-white border border-gray-300 overflow-hidden flex-1 flex flex-col">
          ...
          <div className="grid gap-2 overflow-auto p-3 md:hidden">
            {pageSuppliers.length === 0 ? <div className="py-12 text-center text-sm text-gray-400">No suppliers found.</div> : pageSuppliers.map((supplier) => (
              <div key={supplier._id} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-gray-900">{supplier.supplier_name}</p><p className="mt-1 text-xs text-gray-500">{supplier.person_name || "No contact person"} · {supplier.city || "No city"}</p></div><StatusBadge active={supplier.isActive} /></div><div className="mt-3 flex items-center justify-between border-t border-gray-100 pt-3"><a href={supplier.phone_number ? `tel:${supplier.phone_number}` : undefined} className="text-sm font-medium text-teal-700">{supplier.phone_number || "No phone"}</a><div className="flex gap-2"><button type="button" onClick={() => setFormModal({ isOpen: true, supplier })} className="rounded-lg bg-gray-100 px-3 py-2 text-xs font-semibold text-gray-700">Edit</button><button type="button" onClick={() => setStatusTarget(supplier)} className={`rounded-lg px-3 py-2 text-xs font-semibold ${supplier.isActive ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-700"}`}>{supplier.isActive ? "Deactivate" : "Activate"}</button></div></div></div>
            ))}
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-200">
                {pageSuppliers.length === 0 ? (
                  <tr><td colSpan={8} className="px-7 py-16 text-center text-sm text-gray-400">No suppliers found.</td></tr>
                ) : pageSuppliers.map((supplier, index) => (
                  <tr key={supplier._id} onClick={() => setDetailsTarget(supplier)} className="cursor-pointer hover:bg-teal-50/40">
                    <td className="px-5 py-4 text-sm font-medium text-gray-500">{(currentPage - 1) * PAGE_SIZE + index + 1}</td>
                    <td className="px-5 py-4 text-sm font-semibold text-gray-800">{supplier.supplier_name}</td>
                    <td className="px-5 py-4 text-sm text-gray-600">{supplier.person_name}</td>
                    <td className="px-5 py-4 text-sm text-gray-700" dir="rtl" lang="ur">{supplier.urdu_title}</td>
                    <td className="px-5 py-4 text-sm text-gray-600">{supplier.phone_number || "-"}</td>
                    <td className="px-5 py-4 text-sm text-gray-600">{supplier.city}</td>
                    <td className="px-5 py-4"><StatusBadge active={supplier.isActive} /></td>
                    <td className="px-5 py-4 text-right relative">
                      <button onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === supplier._id ? null : supplier._id); }} className="p-2 text-gray-400 hover:text-gray-900 rounded-lg hover:bg-gray-100" aria-label="Open actions menu">
                        <MoreVertical size={18} />
                      </button>
                      <ContextMenu isOpen={activeMenu === supplier._id}>
                        <button onClick={(e) => { e.stopPropagation(); setFormModal({ isOpen: true, supplier }); setActiveMenu(null); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl text-gray-600 hover:bg-gray-200 cursor-pointer">
                          <Edit3 size={16} strokeWidth={2.5} /> Edit Supplier
                        </button>
                        <div className="h-[1px] bg-gray-200 my-1.5" />
                        <button onClick={(e) => { e.stopPropagation(); setStatusTarget(supplier); setActiveMenu(null); }} className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl cursor-pointer ${supplier.isActive ? "text-red-600 hover:bg-red-50" : "text-emerald-700 hover:bg-emerald-50"}`}>
                          <UserRoundCheck size={16} strokeWidth={2.5} /> {supplier.isActive ? "Deactivate" : "Activate"}
                        </button>
                      </ContextMenu>
                    </td>
                  </tr>
