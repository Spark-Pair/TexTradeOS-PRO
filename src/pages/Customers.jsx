...
export default function Customers() {
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      ...
      showToast({ type: "error", message: errorMessage(error, "Could not save customer") });
      throw error;
    }
  };
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      showToast({ type: "error", message: errorMessage(error, "Could not change customer status") });
    } finally { setIsStatusChanging(false); }
  };

  const filterConfig = [
    { label: "Customer", placeholder: "Search customer or person", type: "text", value: filters.name, onChange: (e) => setFilters((prev) => ({ ...prev, name: e.target.value })) },
