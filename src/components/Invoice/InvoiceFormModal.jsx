...
const formatInvoiceNumber = (year, next) =>
  `${year}-${String(Math.max(1, Number(next) || 1)).padStart(4, "0")}`;
const toInvoiceInventory = (items = []) =>
  items
    .map((item) => ({
      ...item,
...
const discountDetails = (discount, pcs, rate) => {
  ...
  const pieces = Math.max(0, numberValue(pcs)),
    unitRate = Math.max(0, numberValue(rate));
  if (!pieces || !unitRate)
    return { amount: 0, type: raw.endsWith("%") ? "percent" : "rupee" };
  if (raw.endsWith("%")) {
    const p = Math.min(100, Math.max(0, numberValue(raw.slice(0, -1))));
    return { amount: pieces * ((unitRate * p) / 100), type: "percent" };
  }
  return {
...
const calculateArticle = (row) => {
  const dzn = numberValue(row.dzn),
    ...
    gross = pcs * rate,
    discount = discountDetails(row.discount, pcs, rate),
    discountAmount = Math.min(gross, discount.amount);
  return {
    ...row,
    dzn,
    pcs,
...
const calculateInvoiceTotals = (rows, returnAmount = 0, receivedAmount = 0) => {
  const gross_amount = rows.reduce(
      (s, r) => s + Number(r.gross_amount || 0),
      0,
    ),
    percent_discount_amount = rows.reduce(
      (s, r) =>
        s +
        (r.discount_type === "percent" ? Number(r.discount_amount || 0) : 0),
      0,
    ),
    rupee_discount_amount = rows.reduce(
      (s, r) =>
        s + (r.discount_type === "rupee" ? Number(r.discount_amount || 0) : 0),
      0,
    ),
    total_discount_amount = percent_discount_amount + rupee_discount_amount,
    net_amount = Math.max(0, gross_amount - total_discount_amount),
    sales_return_amount = Math.min(
      net_amount,
      Math.max(0, numberValue(returnAmount)),
    ),
    total_amount = Math.max(0, net_amount - sales_return_amount),
    received_amount = Math.max(0, numberValue(receivedAmount));
  return {
    gross_amount,
    ...
    received_amount,
    total_amount,
    balance_amount: Math.max(0, total_amount - received_amount),
    return_amount: Math.max(0, received_amount - total_amount),
  };
};
...
function InvoiceItemModal({
  ...
  return (
      ...
      footer={
        <div className="flex w-full items-center justify-between gap-3">
          ...
          <div className="flex gap-3">
            <Button outline variant="secondary" onClick={onClose}>
              Discard
            </Button>
            <Button icon={Save} onClick={submit}>
              {article ? "Save Item" : "Add Item"}
            </Button>
          </div>
        </div>
      }
    >
      <form
        onSubmit={submit}
        className="grid grid-cols-1 gap-3.5 p-0.5 md:grid-cols-3"
      >
        <div className="md:col-span-3">
          ...
          {selectedArticle && (
            <div className="mt-2 grid gap-2 rounded-xl border border-gray-300 bg-gray-50 p-3 text-xs text-gray-600 md:grid-cols-3">
              <p>
                <b className="text-gray-800">Article No:</b>{" "}
                {selectedArticle.article_no}
              </p>
              <p>
                <b className="text-gray-800">Available:</b>{" "}
                {selectedArticle.stock_pcs || 0} pcs (
                {selectedArticle.unit
                  ? (selectedArticle.stock_pcs / selectedArticle.unit).toFixed(
                      2,
                    )
                  : 0}{" "}
                pkt)
              ...
              <p>
                <b className="text-gray-800">Sale Rate:</b>{" "}
                {Number(
                  selectedArticle.sale_rate || selectedArticle.rate || 0,
                ).toFixed(2)}
              </p>
            </div>
          )}
        ...
        <Input
          ...
          value={row?.quantity_pkt || ""}
          onChange={(e) => update("quantity_pkt", e.target.value)}
          placeholder="e.g. 2"
        />
        <Input
          label="Dozens"
        ...
        <Input
          label="Amount"
          value={row ? calculateArticle(row).amount.toFixed(2) : "0.00"}
          disabled
        />
...
export default function InvoiceFormModal({ isOpen, onClose, onAction }) {
  ...
  const [step, setStep] = useState("entry"),
    [entryStage, setEntryStage] = useState(1),
    [customerId, setCustomerId] = useState(""),
    [customerMode, setCustomerMode] = useState("registered"),
    [quickCustomerOpen, setQuickCustomerOpen] = useState(false),
    [walkIn, setWalkIn] = useState({
    ...
    [salesmanName, setSalesmanName] = useState(""),
    [articles, setArticles] = useState([]),
    [inventory, setInventory] = useState([]),
    [itemModal, setItemModal] = useState({ isOpen: false, article: null }),
    [scanModalOpen, setScanModalOpen] = useState(false),
    [salesReturnRows, setSalesReturnRows] = useState([]),
    [includeSalesReturn, setIncludeSalesReturn] = useState(false),
    [returnAdjustment, setReturnAdjustment] = useState({
      type: "none",
      value: "",
    }),
  ...
  const [salesReturnInventory, setSalesReturnInventory] = useState([]),
    [salesReturnLoading, setSalesReturnLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([customerRows, inventoryRows, counter]) => {
        if (cancelled) return;
        setCustomers(
          (Array.isArray(customerRows) ? customerRows : []).filter(
            (c) => c.isActive !== false,
          ),
        );
        setInventory(
          toInvoiceInventory(Array.isArray(inventoryRows) ? inventoryRows : []),
        );
        setInvoiceNumber(
          formatInvoiceNumber(
            counter?.year || new Date().getFullYear(),
            counter?.next_invoice_no || 1,
          ),
        );
      })
      .catch((err) => {
        if (!cancelled) {
          ...
          setError(
            err?.response?.data?.message ||
              "Could not load invoice data from the server.",
          );
        }
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetchSalesReturnable(customerId)
      .then((items) => {
        if (!cancelled)
          setSalesReturnInventory(Array.isArray(items) ? items : []);
      })
      .catch((err) => {
        if (!cancelled) {
          setSalesReturnInventory([]);
          setError(
            err?.response?.data?.message ||
              "Could not load returnable articles for this customer.",
          );
        }
      })
      .finally(() => {
        if (!cancelled) setSalesReturnLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  const calculatedArticles = useMemo(
    () => articles.map(calculateArticle),
    [articles],
  );
  const itemInventory = useMemo(
    () =>
      inventory
        .map((item) => {
          const used = articles.reduce(
              (s, r) =>
                r.article_no === item.article_no &&
                r._key !== itemModal.article?._key
                  ? s + numberValue(r.pcs)
                  : s,
              0,
            ),
            stock = Math.max(0, numberValue(item.stock_pcs) - used);
          return {
            ...item,
            stock_pcs: stock,
            label: `${item.article_no} | ${item.description || "Article"} | Available ${stock} pcs | Sale ${item.sale_rate || item.rate || 0}`,
          };
        })
        .filter(
          (item) =>
    ...
    () =>
      inventory
        .map((item) => ({
          ...
          stock_pcs: Math.max(
            0,
            numberValue(item.stock_pcs) -
              articles.reduce(
                (s, r) =>
                  r.article_no === item.article_no ? s + numberValue(r.pcs) : s,
                0,
              ),
          ),
        }))
  ...
  const salesReturnTotals = useMemo(
    () => returnTotals(salesReturnRows, returnAdjustment),
    [salesReturnRows, returnAdjustment],
  );
  const invoiceTotals = useMemo(
    () =>
      calculateInvoiceTotals(
        calculatedArticles,
        salesReturnTotals.amount,
        payment.received_now ? payment.amount : 0,
      ),
    [calculatedArticles, salesReturnTotals.amount, payment],
  );
    ...
    () =>
      customerMode === "walkin"
        ? {
            _id: "",
            ...walkIn,
            customer_name: walkIn.customer_name.trim() || "Walk-in Customer",
            isWalkIn: true,
          }
  ...
  const draftInvoice = useMemo(
    () => ({
      ...
      sales_return: {
        articles: salesReturnRows,
        adjustment: returnAdjustment,
        total_pcs: salesReturnTotals.pcs,
        gross_amount: salesReturnTotals.gross,
        adjustment_amount: salesReturnTotals.adjustment,
        total_amount: salesReturnTotals.amount,
      },
      payment: {
        ...payment,
        amount: payment.received_now ? numberValue(payment.amount) : 0,
      },
      ...invoiceTotals,
    }),
    [
  ...
  const handleQuickCustomerCreate = async (payload) => {
    try {
      const created = await createCustomer(payload);
      const rows = await fetchCustomers();
      setCustomers(
        (Array.isArray(rows) ? rows : []).filter((c) => c.isActive !== false),
      );
      setCustomerMode("registered");
      ...
      showToast({
        type: "success",
        message: `${created.customer_name} added and selected.`,
      });
      return created;
    } catch (err) {
          ...
          i === duplicate
            ? calculateArticle(
                syncQuantity(
                  r,
                  "pcs",
                  String(numberValue(r.pcs) + numberValue(article.pcs)),
                ),
              )
            : r,
        );
  ...
  const mergeScannedRows = (incoming) => {
    setArticles((current) => {
      const next = [...current];
      incoming.forEach((row) => {
        const index = next.findIndex(
          (item) => item.article_no === row.article_no,
        );
        if (index >= 0)
          next[index] = calculateArticle(
            syncQuantity(
              next[index],
              "pcs",
              String(numberValue(next[index].pcs) + numberValue(row.pcs)),
            ),
          );
        else next.push(row);
      });
  ...
  const removeRow = (key) =>
    setArticles((rows) => rows.filter((r) => r._key !== key));
  const advance = () => {
    if (entryStage === 1 && !selectedCustomer)
      return setError("Select a customer to continue.");
    if (entryStage === 1 && !salesmanName.trim())
      return setError("Enter the salesperson name to continue.");
    if (entryStage === 2 && !calculatedArticles.length)
      return setError("Add at least one article to continue.");
    setError("");
    setEntryStage((s) => Math.min(3, s + 1));
  };
  const review = () => {
    if (!selectedCustomer) return setError("Select a customer.");
    if (!salesmanName.trim()) return setError("Salesman name is required.");
    ...
    if (over)
      return setError(
        `${over.article_no}: only ${numberValue(inventory.find((i) => i.article_no === over.article_no)?.stock_pcs)} pieces are available.`,
      );
    setError("");
    setStep("preview");
  };
  ...
  return (
    <Modal
      ...
      subtitle={
        step === "entry"
          ? `Invoice ${invoiceNumber || "..."} · Complete the 3 simple sections below`
          : "Check customer, items and payment before saving"
      }
      maxWidth="max-w-5xl"
        ...
        <div className="grid gap-5 p-0.5">
          <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-gray-200 bg-gray-50 text-center text-[11px] font-semibold sm:text-xs">
            <button
              ...
              className={`border-r border-gray-200 px-2 py-2.5 ${entryStage === 1 ? "bg-teal-50 text-teal-700" : "bg-white text-gray-700"}`}
            >
              1 · Customer
            </button>
            <button
              type="button"
              disabled={entryStage < 2}
              onClick={() => setEntryStage(2)}
              className={`border-r border-gray-200 px-2 py-2.5 ${entryStage === 2 ? "bg-teal-50 text-teal-700" : "text-gray-400"}`}
            >
              2 · Items
            </button>
            <button
              type="button"
              disabled={entryStage < 3}
              onClick={() => setEntryStage(3)}
              className={`px-2 py-2.5 ${entryStage === 3 ? "bg-teal-50 text-teal-700" : "text-gray-400"}`}
            >
              3 · Payment
            </button>
          </div>
          {loadingData && (
            <p className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm text-gray-500">
              Loading customers and inventory from server...
            </p>
          )}
          <section className={entryStage === 1 ? "block" : "hidden"}>
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div className="relative grid w-full grid-cols-2 rounded-xl border border-gray-300 bg-gray-100 p-1 sm:w-[270px]">
                ...
                <button
                  ...
                  className={`relative z-10 rounded-lg px-3 py-2 text-xs font-semibold ${customerMode === "registered" ? "text-gray-950" : "text-gray-500"}`}
                >
                  Registered
                </button>
                <button
                  ...
                  className={`relative z-10 rounded-lg px-3 py-2 text-xs font-semibold ${customerMode === "walkin" ? "text-gray-950" : "text-gray-500"}`}
                >
                  Walk-in
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 gap-3.5 md:grid-cols-2">
              {customerMode === "registered" ? (
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setWalkIn((w) => ({ ...w, customer_name: e.target.value }))
                  }
                  required={false}
                />
              )}
              <Input
                ...
                value={salesmanName}
                onChange={(e) =>
                  setSalesmanName(capitalizeWords(e.target.value))
                }
            ...
            {customerMode === "walkin" && (
              <div className="mt-3 grid grid-cols-1 gap-3.5 rounded-xl border border-gray-200 bg-gray-50 p-3 md:grid-cols-3">
                <Input
                  ...
                  onChange={(e) =>
                    setWalkIn((w) => ({ ...w, phone_number: e.target.value }))
                  }
                  required={false}
                />
                <Input
                  label="Person Name"
                  value={walkIn.person_name}
                  onChange={(e) =>
                    setWalkIn((w) => ({ ...w, person_name: e.target.value }))
                  }
                  required={false}
                />
                <Input
                  ...
                  onChange={(e) =>
                    setWalkIn((w) => ({ ...w, address: e.target.value }))
                  }
                  required={false}
                />
              </div>
            ...
            {selectedCustomer && customerMode === "registered" && (
              <div className="mt-3 grid gap-2 rounded-xl border border-gray-300 bg-gray-50 p-3 text-xs text-gray-600 md:grid-cols-4">
                <p>
                  <b className="text-gray-800">Person:</b>{" "}
                  {selectedCustomer.person_name || "-"}
                </p>
                <p lang="ur">
                  <b className="text-gray-800">Urdu:</b>{" "}
                  {selectedCustomer.urdu_title || "-"}
                </p>
                <p>
                ...
                <p>
                  <b className="text-gray-800">City:</b>{" "}
              ...
              right={
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    icon={Plus}
                    onClick={() =>
                      setItemModal({ isOpen: true, article: null })
                    }
                  >
                    Add Item
                  </Button>
              ...
              {calculatedArticles.length === 0 ? (
                <div className="rounded-xl border border-dashed border-gray-300 px-4 py-8 text-center">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    Tap Add Item, or scan the packet label.
                  </p>
                </div>
              ) : (
                calculatedArticles.map((a) => (
                  <div
                    key={a._key}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex gap-1">
                        <button
                          type="button"
                          onClick={() =>
                            setItemModal({ isOpen: true, article: a })
                          }
                    ...
                    <div className="mt-3 grid grid-cols-4 gap-2 border-t border-gray-100 pt-3 text-xs">
                      <div>
                        <span className="text-gray-400">Packets</span>
                        <p className="font-semibold">{a.quantity_pkt || 0}</p>
                      </div>
                      <div>
                        <span className="text-gray-400">Pieces</span>
                        <p className="font-semibold">{a.pcs || 0}</p>
                      </div>
                      <div>
                        <span className="text-gray-400">Rate</span>
                        <p className="font-semibold">{a.rate.toFixed(2)}</p>
                      </div>
              ...
              <table className="w-full min-w-[920px] border-collapse">
                <thead>
                  <tr className="border-b border-gray-300 bg-gray-50 text-xs font-semibold text-gray-500">
                    ...
                    <th className="px-2 py-2.5">Pckt</th>
                    <th className="px-2 py-2.5">Dzn</th>
                    <th className="px-2 py-2.5">Pieces</th>
                    <th className="px-2 py-2.5">Sale Rate</th>
                    <th className="px-2 py-2.5">Discount</th>
                    <th className="px-3 py-2.5 text-right">Amount</th>
                    <th className="w-24 px-2 py-2.5">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {calculatedArticles.length === 0 ? (
                    <tr>
                      <td
                        colSpan={11}
                        className="px-7 py-10 text-center text-sm text-gray-400"
                      >
                        No items added yet. Add an article manually or scan its
                        QR label.
                      </td>
                    </tr>
                  ) : (
                    calculatedArticles.map((a, index) => (
                      <tr
                        key={a._key}
                        className="border-b border-gray-200 hover:bg-emerald-50/30"
                      >
                ...
                <tfoot>
                  <tr className="bg-gray-50 font-semibold">
                    ...
                    <td className="px-3 py-3 text-right text-sm text-emerald-700">
                      {invoiceTotals.net_amount.toFixed(2)}
                    </td>
                    <td />
                  </tr>
                </tfoot>
          ...
          <section className={entryStage === 3 ? "block" : "hidden"}>
            <SectionHeader
              ...
              subtitle="Record any sales return and the payment received with this invoice"
            />
            <div className="grid gap-5">
                ...
                <div className="flex items-center justify-between gap-3 bg-gray-50 px-4 py-3">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-800">
                        Sales Return with this Invoice
                      </p>
                      <p className="text-xs text-gray-400">
                        Optional — return previously sold goods and adjust
                        payable.
                      </p>
                    </div>
                  </div>
                  {customerMode === "registered" ? (
                    ...
                    <span className="text-xs font-medium text-gray-400">
                      Not available for walk-in sale
                    </span>
                  )}
                </div>
                {customerMode === "registered" && includeSalesReturn && (
                  <div className="border-t border-gray-200 p-4">
                    <ReturnEditor
                      title="Returned Articles"
                      subtitle={
                        salesReturnLoading
                          ? "Loading customer sold articles..."
                          : "Customer sold articles"
                      }
                      inventory={salesReturnInventory}
                      rows={salesReturnRows}
                      onChange={setSalesReturnRows}
                      adjustment={returnAdjustment}
                      onAdjustmentChange={setReturnAdjustment}
                    />
                  </div>
                )}
              </div>
              <div className="border-t border-gray-200 pt-4">
                <SectionHeader
                  ...
                  subtitle="Record payment received against the final payable amount"
                />
                <PaymentEditor payment={payment} onChange={setPayment} />
              </div>
              <div className="grid grid-cols-2 gap-2 rounded-xl border border-gray-300 bg-gray-50 p-3 text-sm md:grid-cols-6">
                ...
                <div>
                  <span className="text-gray-500">Return PCs</span>
                  <p className="font-semibold">{salesReturnTotals.pcs}</p>
                </div>
                <div>
                  <span className="text-gray-500">Return</span>
                  <p className="font-semibold text-red-600">
                    -{salesReturnTotals.amount.toFixed(2)}
                  </p>
                </div>
                <div>
                  <span className="text-gray-500">Payable</span>
                  <p className="font-semibold text-emerald-700">
                    {invoiceTotals.total_amount.toFixed(2)}
                  </p>
                </div>
                <div>
                  <span className="text-gray-500">Received</span>
                  <p className="font-semibold text-sky-700">
                    {invoiceTotals.received_amount.toFixed(2)}
                  </p>
                </div>
                <div className="rounded-lg bg-white px-3 py-2 ring-1 ring-gray-200">
                  <span className="text-gray-500">
                    {invoiceTotals.return_amount > 0
                      ? "Change / Advance"
                      : "Balance Due"}
                  </span>
                  <p
                    className={`text-base font-bold ${invoiceTotals.return_amount > 0 ? "text-emerald-700" : "text-red-600"}`}
                  >
                    {(
                      invoiceTotals.return_amount ||
                      invoiceTotals.balance_amount
                    ).toFixed(2)}
                  </p>
                </div>
              </div>
