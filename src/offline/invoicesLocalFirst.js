...
export const createInvoiceLocalFirst = async (payload) => {
  ...
  if (Array.isArray(payload?.articles)) {
    const articles = payload.articles.map((article) => ({
      ...article,
      _key: article._key || crypto.randomUUID(),
      dzn: Number(article.dzn || 0),
      pcs: Number(article.pcs || 0),
      rate: Number(article.rate || 0),
      discount_amount: Number(article.discount_amount || 0),
      amount: Number(article.amount || 0),
    }));
    const localInvoice = {
      _id: localId,
